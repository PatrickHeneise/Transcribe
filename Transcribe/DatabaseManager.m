...
#define DATABASE_NAME @"transcribe"
...
-(void)updateDocument:(CCouchDBDocument *)inDocument
{
	...
	CURLOperation *op = [self.database operationToUpdateDocument:inDocument 
							   successHandler:inSuccessHandler 
							   failureHandler:inFailureHandler];
	[op start];
}
...
-(void)uploadDocument
{
	...
	[self syncFrom:@"transcribe" to:@"http://transcribe:ebircsnart@transcribe.iriscouch.com/transcribe" onSuccess:successHandler onError:errorHandler];
}
